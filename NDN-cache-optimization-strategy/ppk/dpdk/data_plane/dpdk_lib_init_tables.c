...
{
//    debug("\033[1;32m""dpdk_lib_init-tables.c: The value of NU_TABLES is %d, NB_REPLICA is %d\n""\033[0m", NB_TABLES,NB_REPLICA);
...
void create_table_on_lcore(unsigned lcore_id)
{
    ...
    {
//        debug("dpdk_lib_init-tables.c:locre %u isn't enabled \n",lcore_id);
        return;
    } 

    int socketid = get_socketid(lcore_id);
//    debug("dpdk_lib_init-tables.c: the value of socketid is %d\n",socketid);
    //state is defined in dpdk_lib.c
    if (state[socketid].tables[0][0] == NULL) {
//        debug("dpdk_lib_init-tables.c: Begin create_tables_on_socket()\n");
...
void init_tables()
{
    debug("dpdk-lib-init-tables.c: Begin to init-tables()\n");
#ifdef PPK_DEBUG
    ...
    debug("%s\n", table_names);
#endif
//    debug("dpdk-lib-init-tables.c: the value of RTE_MAX_LCORE is %u\n",RTE_MAX_LCORE);
