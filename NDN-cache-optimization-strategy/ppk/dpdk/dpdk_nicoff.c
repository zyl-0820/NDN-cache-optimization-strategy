...
testcase_t* current_test_case;
...
#ifdef PPK_TEST_SUITE
    extern testcase_t ppk_test_suite[MAX_TESTCASES];
    ...
    void ppk_pre_launch(int idx) {
        ...
        current_test_case = ppk_test_suite + idx;
    ...
    int launch_count() {
        int idx = 0;
        testcase_t* suite = ppk_test_suite;
        while (suite[idx].steps)   ++idx;
        return idx;
    }
#else
    #ifndef PPK_TESTCASE
    #error "One of these two macros must be defined: PPK_TEST_SUITE, PPK_TESTCASE"
    #endif

    #define TOSTRING(x) #x

    extern fake_cmd_t PPK_TESTCASE[][RTE_MAX_LCORE];

    testcase_t single_test_case = {
        .name  = TOSTRING(PPK_TESTCASE),
        .steps = &PPK_TESTCASE,
    };
