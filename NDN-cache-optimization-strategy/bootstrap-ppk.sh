...
PPK_DIR=${PPK_DIR-ppk}
PPK_BUILD_DIR=${PPK_BUILD_DIR-"${PPK_DIR}/build"}
...
REPO_PATH_P4Runtime_GRPCPP_tang="https://gitee.com/tkhfree/p4-runtime_-grpcpp"
...
if [ "$INSTALL_STAGE6_PPK" == "yes" ]; then
    [ ! -d ppk ] && git clone "$REPO_PATH_ppk" --no-hardlinks --recursive $PPK_CLONE_OPT >$(logfile "get-ppk") 2>&1 &
    WAITPROC_PPK="$!"
...
if [ "$INSTALL_STAGE5_GRPC" == "yes" ]; then
    ...
    [ $ISSKIP -ne 1 ] && CC="ccache ${PPK_CC}" CC_LD="${PPK_LD}" CXX="${PPK_CXX}" ./configure --with-proto >>$(logfile "PI" ".configure") 2>&1
