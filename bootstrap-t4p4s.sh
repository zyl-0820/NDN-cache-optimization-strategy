...
if [ $# -gt 0 ] && [ "$1" == "showenvs" ]; then
    ...
    echo "    ${colours[0]}MAX_MAKE_JOBS$nn=${colours[1]}8$nn ${colours[0]}PPK_CC$nn=${colours[1]}clang$nn ${colours[0]}$0$nn"
...
PPK_DIR=${PPK_DIR-ppk}
PPK_BUILD_DIR=${PPK_BUILD_DIR-build}

find_tool() {
    SEP=$1
    shift
    DEFAULT_TOOL=$1
    PPK_TOOL_DIR="${PPK_BUILD_DIR}/tools"
    mkdir -p "${PPK_TOOL_DIR}"
    TOOL_FILE="${PPK_TOOL_DIR}/tool.${DEFAULT_TOOL}.txt"
...
PPK_CC=${PPK_CC-$(find_tool "-" clang clang gcc gcc)}
if [[ ! "$PPK_CC" =~ "clang" ]]; then
    # note: when using gcc, only lld seems to be supported, not lld-VSN
    PPK_LD=${PPK_LD-$(find_tool "" lld lld bfd bfd gold gold)}
else
    PPK_LD=${PPK_LD-$(find_tool "-" lld lld bfd bfd gold gold)}
fi

PPK_CXX=${PPK_CXX-$(find_tool "-" "clang++" clang "g++" "g++")}

MISSING_TOOLS=""
[ "$PYTHON3"   == "" ] && MISSING_TOOLS="${MISSING_TOOLS} Python3"
[ "$PPK_CC"  == "" ] && MISSING_TOOLS="${MISSING_TOOLS} CC"
[ "$PPK_CXX" == "" ] && MISSING_TOOLS="${MISSING_TOOLS} CXX"
[ "$PPK_LD"  == "" ] && MISSING_TOOLS="${MISSING_TOOLS} LD"

FOUND_TOOLS=""
[ "$PYTHON3"   != "" ] && FOUND_TOOLS="${FOUND_TOOLS} PYTHON3=${cc}${PYTHON3}$nn"
[ "$PPK_CC"  != "" ] && FOUND_TOOLS="${FOUND_TOOLS} CC=${cc}${PPK_CC}$nn"
[ "$PPK_CXX" != "" ] && FOUND_TOOLS="${FOUND_TOOLS} CXX=${cc}${PPK_CXX}$nn"
[ "$PPK_LD"  != "" ] && FOUND_TOOLS="${FOUND_TOOLS} LD=${cc}${PPK_LD}$nn"
...
INSTALL_STAGE6_PPK=${INSTALL_STAGE6_PPK-yes}

PROTOBUF_USE_RC=${PROTOBUF_USE_RC-no}
TODAY=`date +%F`
P4C_COMMIT_DATE=${P4C_COMMIT_DATE-$TODAY}

PPK_ENVVAR_FILE=ppk_env.sh

REPO_PATH_protobuf_tang="https://gitee.com/tkhfree/protobuf"
REPO_PATH_p4c_tang="https://gitee.com/tkhfree/p4c"
REPO_PATH_grpc_tang="https://gitee.com/tkhfree/grpc"
REPO_PATH_PI_tang="https://gitee.com/tkhfree/PI"
REPO_PATH_P4Runtime_GRPCPP_tang="https://gitee.com/tkhfree/p4runtime_grpcpp.git"
REPO_PATH_ppk_tang="https://gitee.com/tkhfree/ppk"

REPO_PATH_protobuf=${REPO_PATH_protobuf_tang-"https://github.com/google/protobuf"}
REPO_PATH_p4c=${REPO_PATH_p4c_tang-"https://github.com/p4lang/p4c"}
REPO_PATH_grpc=${REPO_PATH_grpc_tang-"https://github.com/grpc/grpc"}
REPO_PATH_PI=${REPO_PATH_PI_tang-"https://github.com/p4lang/PI"}
REPO_PATH_P4Runtime_GRPCPP=${REPO_PATH_P4Runtime_GRPCPP_tang-"https://github.com/P4ELTE/P4Runtime_GRPCPP"}
REPO_PATH_ppk=${REPO_PATH_ppk_tang-"https://github.com/P4ELTE/t4p4s"}
...
TOOL_VSN_CFG_FILE=${TOOL_VSN_CFG_FILE-"bootstrap-ppk-tools-vsn.cfg"}
...
#set var,for DPDK_VSN=19.11 / P4C_COMMIT_DATE=2021-11-01 / PROTOBUF_TAG=v3.8.0
...
if [ "$CLEANUP" == "yes" ]; then
    ...
    mv --backup=numbered ppk* $CLEANUP_DIR/ 2>/dev/null
    mv --backup=numbered ${PPK_ENVVAR_FILE} $CLEANUP_DIR/ 2>/dev/null
...
APPROX_TOTAL_MB=$(($APPROX_INSTALL_MB+INST_MB_PPK_EXAMPLES))
...
if [ "$RTE_TARGET" != "" ]; then
    ...
    export RTE_TARGET=${RTE_TARGET-"x86_64-native-linuxapp-$PPK_CC"}
fi

echo -e "Using ${cc}p4c$nn commit from ${cc}$P4C_COMMIT_DATE$nn"

if [ `echo $PYTHON|cut -d ' ' -f 2 | cut -d '.' -f 2` == 5 ]; then
PKGS_PYTHON="${PYTHON3} ${PYTHON3}-dev python3-dill python3-setuptools"
wget https://bootstrap.pypa.io/pip/3.5/get-pip.py
$PYTHON get-pip.py
else
PKGS_PYTHON="${PYTHON3} ${PYTHON3}-dev python3-dill python3-setuptools python3-pip"
fi
...
PKGS_MAKE="re2c automake bison flex cmake ccache  pkg-config"
PKGS_GRPC=""
...
REQUIRED_PACKAGES="$PKGS_PYTHON $PKGS_LIB $PKGS_MAKE $PKGS_GRPC g++ tcpdump golang"
PIP_PACKAGES="meson more-itertools pyelftools pybind11 pysimdjson dill"
if [ "$USE_OPTIONAL_PACKAGES" == "yes" ]; then
    OPT_PACKAGES="python3-ipdb python3-termcolor  python3-yaml python3-ujson python3-ruamel.yaml gnome-terminal"
    PIP_PACKAGES="$PIP_PACKAGES backtrace"
fi

#. ./boot.sh branchname
# [ $# -gt 0 ] && PPK_DIR="t4p4s-$1" && PPK_CLONE_OPT="$PPK_DIR -b $1" && echo -e "Using the $cc$1$nn branch of PPK"
...
if [ "$LOCAL_REPO_CACHE" != "" ]; then
    ...
    [ ! -d "$LOCAL_REPO_CACHE/ppk" ] && echo -e "${cc}Warning$nn: \$LOCAL_REPO_CACHE/ppk$nn is not a directory, installation will use the remote repo"
    [   -d "$LOCAL_REPO_CACHE/ppk" ] && REPO_PATH_ppk="$LOCAL_REPO_CACHE/ppk"
fi
...
if [ "$INSTALL_STAGE1_PACKAGES" == "yes" ]; then
    ...
    rm -rf ninja
    git clone https://gitee.com/hejuncheng1/ninja.git && cd ninja
    ./configure.py --bootstrap
    sudo cp ./ninja /usr/bin && cd -
fi
...
if [ "$INSTALL_STAGE5_GRPC" == "yes" ]; then
    [ ! -d grpc ] && git clone "$REPO_PATH_grpc" --no-hardlinks >$(logfile "get-grpc") 2>&1 && \
    # --------++++++++++++++++++++++++++++++++++++++++++++++++++++=
    cd grpc && git checkout tags/v1.17.2 && cat <<EOF >.gitmodules
[submodule "third_party/zlib"]
	path = third_party/zlib
	url = https://gitee.com/QQ975150313/zlib.git
...
[submodule "third_party/protobuf"]
	...
	url = https://gitee.com/tkhfree/protobuf.git
	branch = 3.0.x
[submodule "third_party/gflags"]
	path = third_party/gflags
	url = https://gitee.com/anynone/gflags-gflags.git
[submodule "third_party/googletest"]
	path = third_party/googletest
	url = https://gitee.com/meiqicheng1216/googletest.git
[submodule "third_party/boringssl"]
	path = third_party/boringssl
	url = https://gitee.com/zhang1021/boringssl.git
...
[submodule "third_party/cares/cares"]
	...
	url = https://gitee.com/tkxiong/cares.git
	branch = cares-1_12_0
[submodule "third_party/bloaty"]
	path = third_party/bloaty
	url = https://gitee.com/killMA/bloaty.git
[submodule "third_party/abseil-cpp"]
	path = third_party/abseil-cpp
	url = https://gitee.com/tkhfree/abseil-cpp.git
[submodule "third_party/libcxxabi"]
	path = third_party/libcxxabi
	url = https://gitee.com/build-sde/libcxxabi.git
	branch = release_60
[submodule "third_party/libcxx"]
	path = third_party/libcxx
	url = https://gitee.com/anynone/llvm-mirror-libcxx.git
	branch = release_60
[submodule "third_party/data-plane-api"]
	path = third_party/data-plane-api
	url = https://gitee.com/hejuncheng1/data-plane-api.git
[submodule "third_party/googleapis"]
	path = third_party/googleapis
	url = https://gitee.com/anynone/googleapis-googleapis.git
[submodule "third_party/protoc-gen-validate"]
	path = third_party/protoc-gen-validate
	url = https://gitee.com/build-sde/protoc-gen-validate.git
EOF
    git submodule update --init >$(logfile "get-grpc") 2>&1
    # cd grpc && sed -i '/https/s!github.com!hub.fastgit.org!' .gitmodules && git submodule update --init >$(logfile "get-grpc") 2>&1
    cd ${WORKDIR}
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    WAITPROC_GRPC="$!"
    ...
    # cd PI && sed -i '/https/s!github.com!hub.fastgit.org!' .gitmodules && git submodule update --init >$(logfile "get-PI") 2>&1 && cd -
    WAITPROC_PI="$!"
...
if [ "$INSTALL_STAGE6_PPK" == "yes" ]; then
    [ ! -d ppk ] && git clone "$REPO_PATH_ppk" --no-hardlinks --recursive >$(logfile "get-ppk") 2>&1 &
    WAITPROC_PPK="$!"
    [ "$PARALLEL_INSTALL" != "yes" ] && wait "$WAITPROC_PPK" >/dev/null 2>&1
fi
...
ctrl_c_handler() {
    ...
    kill -9 $WAITPROC_PPK 2>/dev/null
...
if [ "$INSTALL_STAGE2_DPDK" == "yes" ]; then
    ...
    cd "$RTE_SDK"
    sudo CC="ccache ${PPK_CC}" CFLAGS="$CFLAGS" CC_LD="${PPK_LD}" $MESONCMD build $MESON_OPTS -Dtests=false -Ddisable_drivers="$DPDK_DISABLED_DRIVERS" >$(logfile "dpdk" ".meson") 2>&1
...
if [ "$INSTALL_STAGE3_PROTOBUF" == "yes" ]; then
    ...
    # cmake .. -DCMAKE_C_FLAGS="$CFLAGS -fPIC ${LTO_OPT}" -DCMAKE_CXX_FLAGS="$CFLAGS -Wno-cpp -fPIC ${LTO_OPT}" -DCMAKE_C_COMPILER="${PPK_CC}" -DCMAKE_CXX_COMPILER="${PPK_CXX}" -GNinja  -DBUILD_TESTS=OFF -DBUILD_CONFORMANCE=OFF -DBUILD_EXAMPLES=OFF -Dprotobuf_BUILD_TESTS=OFF -Dprotobuf_BUILD_CONFORMANCE=OFF -Dprotobuf_BUILD_EXAMPLES=OFF >$(logfile "protobuf" ".cmake") 2>&1
    ...
    # ---------------++++++++++++++++++++++++++++++++++++++++++++++++++++
    cd protobuf
    ./autogen.sh >$(logfile "protobuf" ".autogen") 2>&1
    ERRCODE=$? && [ $ISSKIP -ne 1 ] && [ $ERRCODE -ne 0 ] && ISSKIP=1 && echo -e "${cc}protobuf$nn/${cc}auto$nn step ${ee}failed$nn with error code ${ee}$ERRCODE$nn"
    ./configure >$(logfile "protobuf" ".autogen") 2>&1
    ERRCODE=$? && [ $ISSKIP -ne 1 ] && [ $ERRCODE -ne 0 ] && ISSKIP=1 && echo -e "${cc}protobuf$nn/${cc}configure$nn step ${ee}failed$nn with error code ${ee}$ERRCODE$nn"
    make >$(logfile "protobuf" ".make") 2>&1
    ERRCODE=$? && [ $ISSKIP -ne 1 ] && [ $ERRCODE -ne 0 ] && ISSKIP=1 && echo -e "${cc}protobuf$nn/${cc}make$nn step ${ee}failed$nn with error code ${ee}$ERRCODE$nn"
    sudo make install >$(logfile "protobuf" ".makeinstall") 2>&1
    ERRCODE=$? && [ $ISSKIP -ne 1 ] && [ $ERRCODE -ne 0 ] && ISSKIP=1 && echo -e "${cc}protobuf$nn/${cc}make install$nn step ${ee}failed$nn with error code ${ee}$ERRCODE$nn"
    sudo ldconfig >>$(logfile "protobuf" ".makeinstall") 2>&1
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    cd "$WORKDIR"
fi
...
if [ "$INSTALL_STAGE4_P4C" == "yes" ]; then
    ...
    sed -i "s/-fuse-ld=gold/-fuse-ld=${PPK_LD}/g" ../CMakeLists.txt
    # temporary fix
    #!!!!!!!!!!!!!!!!!!!!! cannot adjust to xxx version
    # sed -i "s/Status::OK/OkStatus()/g" ../control-plane/p4RuntimeSerializer.cpp
    # cmake .. -GNinja -DCMAKE_BUILD_TYPE=MinSizeRel \
    #     -DCMAKE_BUILD_WITH_INSTALL_RPATH=ON \
    #     -DCMAKE_C_COMPILER="gcc" -DCMAKE_CXX_COMPILER="g++" -DCMAKE_LINKER="${PPK_LD}" \
    ...
    # ---------------++++++++++++++++++++++++++++++++++++++++++++++++++++
     [ $ISSKIP -ne 1 ] && sudo make -j ${MAX_MAKE_JOBS_P4C} >&2 2>$(logfile "p4c" ".make")
     ERRCODE=$? && [ $ISSKIP -ne 1 ] && [ $ERRCODE -ne 0 ] && ISSKIP=1 && echo -e "${cc}p4c$nn/${cc}make$nn step ${ee}failed$nn with error code ${ee}$ERRCODE$nn"
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    # ---------------++++++++++++++++++++++++++++++++++++++++++++++++++++
     [ $ISSKIP -ne 1 ] && sudo make install -j ${MAX_MAKE_JOBS_P4C} 2>&1 >$(logfile "p4c" ".make.install")
     ERRCODE=$? && [ $ISSKIP -ne 1 ] && [ $ERRCODE -ne 0 ] && ISSKIP=1 && echo -e "${cc}p4c$nn/${cc}make install$nn step ${ee}failed$nn with error code ${ee}$ERRCODE$nn"
     [ $ISSKIP -ne 1 ] && sudo ldconfig 2>&1 >$(logfile "p4c" ".make.install")
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    # [ $ISSKIP -ne 1 ] && sudo ldconfig 2>&1 >$(logfile "p4c" ".ninja-make.install")
    cd "$WORKDIR"
fi
...
if [ "$INSTALL_STAGE5_GRPC" == "yes" ]; then
    ...
    # ---------------++++++++++++++++++++++++++++++++++++++++++++++++++++
    cd grpc/
    make >$(logfile "grpc" ".make") 2>&1
    ERRCODE=$? && [ $ISSKIP -ne 1 ] && [ $ERRCODE -ne 0 ] && ISSKIP=1 && echo -e "${cc}grpc$nn/${cc}make$nn step ${ee}failed$nn with error code ${ee}$ERRCODE$nn"
    sudo make install >>$(logfile "grpc" ".make") 2>&1
    ERRCODE=$? && [ $ISSKIP -ne 1 ] && [ $ERRCODE -ne 0 ] && ISSKIP=1 && echo -e "${cc}grpc$nn/${cc}make install$nn step ${ee}failed$nn with error code ${ee}$ERRCODE$nn"
    sudo ldconfig >>$(logfile "grpc" ".make") 2>&1
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    # mkdir -p grpc/build
    # cd grpc/build
    # cmake .. -DCMAKE_C_FLAGS="$CFLAGS -fPIC" -DCMAKE_CXX_FLAGS="$CFLAGS -Wno-cpp -fPIC" -DCMAKE_C_COMPILER="${PPK_CC}" -DCMAKE_CXX_COMPILER="${PPK_CXX}" -GNinja >$(logfile "grpc" ".cmake") 2>&1
    ...
    [ $ISSKIP -ne 1 ] && ./configure --with-proto >>$(logfile "PI" ".configure") 2>&1
    ...
    cd p4runtime_grpcpp
    ISSKIP=0
    sudo chmod +x install.sh compile.sh
    ...
    [ $ISSKIP -ne 1 ] && CC="ccache ${PPK_CC}" CC_LD="${PPK_LD}" CXX="${PPK_CXX}" ./compile.sh >>$(logfile "P4Runtime_GRPCPP" ".compile") 2>&1
...
if [ "$INSTALL_STAGE6_PPK" == "yes" ]; then
    [ "$PARALLEL_INSTALL" == "yes" ] && [ -d "/proc/${WAITPROC_PPK}" ] 2>/dev/null && echo -ne "Waiting for ${cc}ppk$nn to download... "
    [ "$PARALLEL_INSTALL" == "yes" ] && wait "$WAITPROC_PPK" >/dev/null 2>&1

    DPDK_FILEVSN=${DPDK_FILEVSN-TO_BE_FILLED_BY_USER}
    RTE_SDK=${RTE_SDK-`pwd`/`ls -d dpdk*$DPDK_FILEVSN*/`}

    cat <<EOF >./${PPK_ENVVAR_FILE}
...
EOF
    ...
    chmod +x ./${PPK_ENVVAR_FILE}
    . ./${PPK_ENVVAR_FILE}

    echo Environment variable config is done
    echo -e "Environment variable config is saved in ${cc}./${PPK_ENVVAR_FILE}$nn"
    ...
    if [ "$ENV_TARGET" == "" ]; then
        echo -e "Make sure you run ${cc}. ./${PPK_ENVVAR_FILE}$nn before you use PPK to setup all environment variables"
    else
        # remove all lines containing "t4p4s_environment_variables"
        sed -ni '/${PPK_ENVVAR_FILE}/!p' ${ENV_TARGET}

        echo >> ${ENV_TARGET}
        echo ". `pwd`/${PPK_ENVVAR_FILE}" >> ${ENV_TARGET}
        echo -e "Environment variable config is ${cc}enabled on login$nn: ${cc}${ENV_TARGET}$nn will run ${cc}${PPK_ENVVAR_FILE}$nn"
    fi
fi
